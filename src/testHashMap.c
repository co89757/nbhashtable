...
/**
 * test the nstring type as key
 */
int testStringKey() {
    Hashtable_t *ht = ht_newHashTable(10);
    int e = 1;
    nstring_t *key = ns_init("one");
    nstring_t *key2 = ns_init("one");
    int eq = TYPE_NSTRING.compare(key, key2);
//    printf("eq = %d\n",eq);
    return eq;
}
...
int main() {
    ...
    if(!testStringKey() )
        printf("NSTRING-COMPARE PASS\n");
    else
        printf("NSTRING-COMPARE TEST FAIL\n");
